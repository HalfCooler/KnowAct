...
先判断原始任务是否已完成，再定位控件。截图验证全部明确要求后禁止继续操作，只能 answer/status；否则执行一个原子动作。
...
coordinate/start_coordinate 必须是长度为 2 的数值数组。禁止 name/message/data/position 等其他 schema，以及变量名、占位符或字符串 "x"/"y"。禁止数组、代码块、解释、思考过程和多个 JSON。
""".strip()
)
